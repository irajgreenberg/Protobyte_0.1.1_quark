...
int jumpID = 0, jumpRing = 0;
float jumpFreq = 0.0f, jumpForce = 0.0;
...
void ProtoJuncusEffusus02::init() {
	...
	for (int r = 0; r < rows; ++r){
		for (int c = 0; c < columns; ++c){
			int ind = r*columns + c;
			juncs.push_back(ProtoJuncusEffusus(Vec3f(0, 0, 0), Vec3f(0, 0, 0), Dim3f(1, 1, 1), Col4f(1, 1, 1, 1), "pink2.jpg", "pink_folds.jpg"));
			// l-r and f-b
			if (columns == 1){
				juncs.at(ind).setPosition(Vec3f(colW*c, 0, -rowH*r));
				juncs.at(ind).setRotation(Vec3f(0, random(-.01, .01), 0));
			}
			else {
	...
	ground = GroundPlane(Vec3f(0, 0/*tubules.at(0).getPosition().y*/, 0), Vec3f(), Dim2f(170, 13490), Col4f(1, 1, 1, 1), 1, 1, "brushed_metal.jpg", .02f);
	//ground = GroundPlane(Vec3f(0, 0/*tubules.at(0).getPosition().y*/, 0), Vec3f(), Dim2f(170, 60), Col4f(.5, .3, .4, 1), 1, 1);
	ground.setTextureScale(.0002f);
	ground.textureOn();
...
void ProtoJuncusEffusus02::run() {
	...
	if (frameCounter++ == 1){
		...
		switch (id){
		case 1:
			juncs.at(0).squeak(ring - 22, freq, force);
			juncs.at(5).squeak(ring - 22, freq, -force);
			break;
		case 2:
			juncs.at(1).squeak(ring - 22, freq, force);
			juncs.at(4).squeak(ring - 22, freq, -force);
			break;
		case 3:
			juncs.at(2).squeak(ring - 22, freq, force);
			juncs.at(7).squeak(ring - 22, freq, -force);
			break;
		case 4:
			juncs.at(3).squeak(ring - 22, freq, force);
			juncs.at(6).squeak(ring - 22, freq, -force);
			break;
		}

		juncs.at(jumpID - 1).pulse(23 - jumpRing, jumpForce);
...
void ProtoJuncusEffusus02::render(int scaleFactor){
	...
	for (int i = 0; i < juncs.size(); ++i){
		...
		S = glm::mat4(1.0f);
		R = glm::rotate(glm::mat4(1.0f), juncs.at(i).getRotation().y, glm::vec3(0, 1, 0));
	...
	const IpEndpointName& remoteEndpoint)
{
	...
	try{
		// example of parsing single messages. osc::OscPacketListener
		// handles the bundle traversal.
		if (strcmp(m.AddressPattern(), "/jumpData") == 0){
			// example #1 -- argument stream interface
			osc::ReceivedMessageArgumentStream args = m.ArgumentStream();
			osc::int32 b1 = 0;
			osc::int32 b2 = 0;
			float b3 = 0;
			args >> b1 >> b2 >> b3 >> osc::EndMessage;
			//trace("the \"REAL\"jumpID =", jumpID);
			trace("b1 = ", b1); // this is embarressing
			//b1 = b1;
