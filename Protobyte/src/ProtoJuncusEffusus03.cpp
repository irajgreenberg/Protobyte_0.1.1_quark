...
void ProtoJuncusEffusus03::init() {
	...
	**************************************/
	...
	for (int i = 0; i < juncsCount; ++i){
		...
		for (int j = 0; j < segs; ++j){
			z = (cos(theta + sin(chi)*random(j))*radius + random(-.04, .04)*j) * (random(1, 3.5) / (j + 1));
			y += step;
			x = (sin(theta + sin(chi)*random(j))*radius + random(-.04, .04)*j) * (random(1, 3.5) / (j + 1));
			cps.push_back(Vec3(x, y, z));
			theta += TWO_PI / segs;
			radius += .085;
			chi += PI / 2;
		}
		juncs.push_back(ProtoJuncusEffusus(Col4f(.7, .7, .6, 1), "lentils.jpg", "leather2.jpg", cps, ProtoJuncusEffusus::DRAFT));
	}
	...
	for (int i = 0; i < juncs.size(); ++i){
		std::vector<Tup4v> temp = juncs.at(i).getGeomData();
...
void ProtoJuncusEffusus03::initUniforms(){
	// lighting
	shader.bind();
...
void ProtoJuncusEffusus03::render(int scaleFactor){
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
		...
		//R = glm::rotate(glm::mat4(1.0f), float(frameCount*.0006), glm::vec3(0, 1, 0));
		//T = glm::translate(glm::mat4(1.0f), glm::vec3(juncs.at(i).getPosition().x, juncs.at(i).getPosition().y, juncs.at(i).getPosition().z - .75f));

		V = glm::lookAt(glm::vec3(0.0, 0.0, 9.0f), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0));
		M = T * R * S;
		MV = V * M;
		MVP = P * MV;
		...
		/*for (int i = 0; i < juncsCount; ++i){
			juncs.at(i).display(SURFACE, SURFACE, 1, 1);
		}*/
