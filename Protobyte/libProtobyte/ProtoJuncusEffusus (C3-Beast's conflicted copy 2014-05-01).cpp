...
void ProtoJuncusEffusus::init(){
	...
	for (size_t i = 0; i < tubuleSpine.getVerts().size(); ++i){
		...
		//ballPosOrigs.push_back(balls.at(i)->getPos());
	}
	...
	for (size_t i = 0; i < balls.size() - 1; ++i){
		...
		// anchor 1
		if (i == 0){
			sticks.push_back(ProtoVerletStick(balls.at(i), balls.at(i + 1), .5, Tup2f(0, .5)));
		}
		// anchor 2
		else if (i == balls.size() - 2){
			sticks.push_back(ProtoVerletStick(balls.at(i), balls.at(i + 1), .5, Tup2f(.5, 0)));
		}
		// body
		else {
			sticks.push_back(ProtoVerletStick(balls.at(i), balls.at(i + 1), .5, Tup2f(.5, .5)));
		}
	}
...
void ProtoJuncusEffusus::pulse(int ring, float force){
	...
	for (int j = 0; j < tubuleCrossSectionDetail*STRIDE; j += STRIDE) {
		...
		tubule.interleavedPrims.at(index) = tubulePrimsOrig.at(index)*max(1, force*2);
		tubule.interleavedPrims.at(index + 1) = tubulePrimsOrig.at(index + 1);
		tubule.interleavedPrims.at(index + 2) = tubulePrimsOrig.at(index + 2)*max(1, force*2);
	}
	//}
	...
	//for (int i = 0; i < tubuleSpineSegments; ++i) {
		for (int j = 0; j < tubuleWrapSegs*STRIDE; j += STRIDE) {
			int index = ring*tubuleWrapSegs*STRIDE + j;
			tubuleWrap.interleavedPrims.at(index) = tubuleWrapPrimsOrig.at(index)*max(1, force * 2);
			tubuleWrap.interleavedPrims.at(index + 1) = tubuleWrapPrimsOrig.at(index + 1);
			tubuleWrap.interleavedPrims.at(index + 2) = tubuleWrapPrimsOrig.at(index + 2)*max(1, force * 2);
		}
	//}
...
void ProtoJuncusEffusus::squeak(int ring, float freq, float force){
//trace("ring =", ring);
	//trace("force =", force);
	// get buffer data form GPU
	glBindBuffer(GL_ARRAY_BUFFER, tubule.getVboID());
	//trace("force =", force);
	//trace("balls.size() =", balls.size());
	//// move verlet ball at ring 
	//if (ring != -1 && ring<14){

	if (ring > 0 && ring < 22){
		balls.at(ring)->pos.x += force*.001;
		balls.at(ring)->pos.z += force*.001;
	}

		//balls.at(ring - 14)->pos = Vec3f(ballPosOrigs.at(ring - 14).x + force*.00000001, ballPosOrigs.at(ring - 14).y, ballPosOrigs.at(ring - 14).z + force*.00000001);
	//}
